...
class Sequentialmodel(nn.Module):
    ...
    def loss_PDE(self, xyz_coll_batch, f_hat_batch):
        ...
        v2_xyz =  autograd.grad(v2,g,torch.ones([xyz_coll_batch.shape[0], 1]).to(device), retain_graph=True, create_graph=True,allow_unused = True)[0]
        w2_xyz =  autograd.grad(w2,g,torch.ones([xyz_coll_batch.shape[0], 1]).to(device), retain_graph=True, create_graph=True,allow_unused = True)[0]
