...
class Sequentialmodel(nn.Module):
    ...
    def __init__(self,vPINN_constants,problem_constants):
        super().__init__() #call __init__ from parent class 
              
        layers = vPINN_constants['layers']
        ...
        beta_mean = 1.0*torch.ones((layers[1],len(layers)-2))
        beta_std = 0.1*torch.ones((layers[1],len(layers)-2))
        
        self.beta1 = Parameter(torch.normal(beta_mean,beta_std))
        self.beta1.requiresGrad = True
        
        self.lb_x = vPINN_constants['lb_x']
        self.ub_x = vPINN_constants['ub_x']

        self.x_test_tensor = vPINN_constants['x_test_tensor']

        self.device = vPINN_constants['device']
        ...
        fvm_results = vPINN_constants['fvm_results']
    ...
    'test neural network'
    def test(self):
        T = self.forward(self.x_test_tensor)[:,0:1]
        C = self.forward(self.x_test_tensor)[:,1:2]
