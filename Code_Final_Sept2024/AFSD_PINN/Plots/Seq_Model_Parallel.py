...
class coupled_PINN(nn.Module):
    ...
    def helper_normalizer(self,f):
        mean_f  = torch.mean(f.detach())
        std_f = torch.mean(f.detach())

        return (f-mean_f)/std_f
    ...
    def helper_gradients(self,g,ind):
        ...
        if(ind==0):    
            ...
            f1 = 1e-6*rho*(u2_x + uv_xyz[:,1] + uw_xyz[:,2]) + p_xyz[:,0] - 2*u_x_xyz[:,0] - (v_x_xyz[:,1] + u_y_xyz[:,1]) - (w_x_xyz[:,2] + u_z_xyz[:,2]) 
            f2 = 1e-6*rho*(uv_xyz[:,0] + v2_y + vw_xyz[:,2])  + p_xyz[:,1] - (u_y_xyz[:,0] + v_x_xyz[:,0]) - 2*v_y_xyz[:,1]  - (w_y_xyz[:,2] + v_z_xyz[:,2])
            f3 = 1e-6*rho*(uw_xyz[:,0] + vw_xyz[:,1] + w2_z)  + p_xyz[:,2] - (u_z_xyz[:,0] + w_x_xyz[:,0]) - (v_z_xyz[:,1] + w_y_xyz[:,1]) - 2*w_z_xyz[:,2]

            # f1 = rho*(u2_x + uv_xyz[:,1] + uw_xyz[:,2])/p_xyz[:,0] + 1  - (2*u_x_xyz[:,0] - (v_x_xyz[:,1] + u_y_xyz[:,1]) - (w_x_xyz[:,2] + u_z_xyz[:,2]))/p_xyz[:,0] 
            # f2 = rho*(uv_xyz[:,0] + v2_y + vw_xyz[:,2])/p_xyz[:,1] + 1 - ((u_y_xyz[:,0] + v_x_xyz[:,0]) - 2*v_y_xyz[:,1]  - (w_y_xyz[:,2] + v_z_xyz[:,2]))/p_xyz[:,1]
            # f3 = rho*(uw_xyz[:,0] + vw_xyz[:,1] + w2_z)/p_xyz[:,2] + 1 - ((u_z_xyz[:,0] + w_x_xyz[:,0]) - (v_z_xyz[:,1] + w_y_xyz[:,1]) - 2*w_z_xyz[:,2])/p_xyz[:,2]
            ...
            # f1 = self.helper_normalizer(f1)
            # f2 = self.helper_normalizer(f2)
            # f3 = self.helper_normalizer(f3)

            # f4 = self.helper_normalizer(f4)
            ...
            if(ind==1):
                ...
                f5 = 1e-3*(kT_xx + kT_yy + kT_zz) + q_g.reshape(-1,) - 1e-3*(rho*c_p)*(uT_x + vT_y + wT_z)*1e-3 #kg/(ms)^3.mm

                # f5 = self.helper_normalizer(f5)
                
                # f5 = (T_xx + T_yy + T_zz)/(rho*c_p*k) - q_g.reshape(-1,)/(rho*c_p*k) - (uT_x + vT_y + wT_z)/k
    ...
    def helper_sigma_eff(self,u_xyz,v_xyz,w_xyz,T):
        ...
        if(torch.mean(T.detach())<200.0):
            # print("Temperature Max:",np.max(T.cpu().detach().numpy())," Temperature Min:",np.min(T.cpu().detach().numpy()))
            log_Z = torch.log(eps_e) + E_a/(R*300.0) #Simplification

            W = (log_Z - log_A)/n

            print(torch.mean(W.cpu().detach()))
            # sigma_e = (1/alpha_sig)*torch.asinh(torch.exp(W)) 
            sigma_e = (1/alpha_sig)*(np.log(2) + W) #Approximation

        else:
            # print("Temperature in correct path...")
            print("Temperature Max:",np.max(T.cpu().detach().numpy())," Temperature Min:",np.min(T.cpu().detach().numpy()))
            log_Z = torch.log(eps_e) + E_a/(R*T)
            W = (log_Z - log_A)/n
            print(torch.mean(W.cpu().detach()))
        ...
        # print(torch.mean(W).cpu().detach().numpy())        
    ...
    def loss_N_top_T(self,xyz_top,N_hat,r_fr_ph_out):
        ...
        # print(delta)
        q_ph = eeta*(0.9*(1-delta)*(sigma_e/np.sqrt(3)) + delta*mu*sigma_e)*(2*pi/(60*1e3))*Omega*r
        q_fr = 0.5*(0.9*(sigma_e/np.sqrt(3)))*(2*pi/(60*1e3))*Omega*r
        
        q = (q_fr*r_fr + q_ph*r_ph + 0*r_out)
        
        f = k*T_xyz[:,2] - 1e3*q.reshape(-1,) #g
        # f = k*T_xyz[:,2] - q.reshape(-1,)
        # f = 1e-3*k*T_xyz[:,2] - 1e3*q.reshape(-1,) #wrong

        # f = self.helper_normalizer(f)
    ...
    def loss_NB_T_5sides(self,xyz_1, xyz_2, xyz_3, xyz_4, xyz_bot):
        ...
        # f = T_z/torch.pow(T_a - T,3) + C_bot/k
        f_true = 0.0*f.detach()
        # f = self.helper_normalizer(f)
        
        loss_5 = self.loss_function(f,f_true)
        
        loss_conv = loss_1 + loss_2 + loss_3 + loss_4 + loss_5
    ...
    def loss(self,xyz_coll,xyz_1, xyz_2, xyz_3, xyz_4,xyz_top,xyz_bot,f_hat,N_hat,uvw_true_top,r_fr_ph_out):
        ...
        loss_T = self.loss_PDE_T(xyz_coll, f_hat.to(self.device2))  + self.loss_N_top_T(xyz_top,N_hat,r_fr_ph_out) + self.loss_NB_T_5sides(xyz_1, xyz_2, xyz_3, xyz_4, xyz_bot)
        ...
        # print(loss_uvw.cpu().detach().numpy())
        # print(loss_T.cpu().detach().numpy())

        return loss_uvw,loss_T

    def pretrain_T_loss(self,xyz_coll):

        T_amb = 298.0*torch.ones([xyz_coll.shape[0], 1]).to(self.device2)
