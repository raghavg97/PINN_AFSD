...
class coupled_PINN(nn.Module):
    ...
    def __init__(self,layers1,layers2,device1,device2,lb_xyz,ub_xyz,sheet_name):
        ...
        # self.PINN_T.k_c = Parameter(torch.tensor(0.001,device = device2))
    ...
    def helper_gradients(self,g,ind):
        ...
        if(ind==0):    
            ...
            if(ind==1):
                ...
                k = k0 - self.PINN_T.k_c*(T.detach()-T_a).reshape(-1,)
                kT_xx = k*T_x_xyz[:,0]
                kT_yy = k*T_y_xyz[:,1]
                kT_zz = k*T_z_xyz[:,2]
...
#         print(loss_B.cpu().detach().numpy())
        ...
        print("Loss ",loss_exp.cpu().detach().numpy(),"C_bot ",self.PINN_T.C_bot.cpu().detach().numpy(),"k_c",self.PINN_T.k_c.cpu().detach().numpy())
