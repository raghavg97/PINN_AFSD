...
class coupled_PINN(nn.Module):
    ...
    def __init__(self,layers1,layers2,device1,device2,lb_xyz,ub_xyz,sheet_name):
        ...
        # self.PINN_T.k_c = Parameter(torch.tensor(0.14,device = device2))
    ...
    def helper_sigma_eff(self,u_xyz,v_xyz,w_xyz,T):
        ...
        if(torch.mean(T.detach())<400.0):
            log_Z = torch.log(eps_e) + E_a/(R*600.0) #Simplification
        else:
            log_Z = torch.log(eps_e) + E_a/(R*T)
    ...
    def loss_NB_T_5sides_v2(self,xyz_1, xyz_2, xyz_3, xyz_4, xyz_bot):
        ...
        T_z = T_xyz[:,2:3]
        k = k0 - self.PINN_T.k_c*(T.detach()-T_a).reshape(-1,1)/1000
        f5 = k*T_z+ self.PINN_T.C_bot*(torch.pow(T_a - T,3)*1e-6) 
        
        # print(f4.shape)
        
        f_conv = torch.cat((f1,f2,f3,f4,f5),dim = 0)
...
#         print(loss_B.cpu().detach().numpy())
        ...
        #     loss_T_full = loss_T
        # else:
        #     loss_T_full = loss_T +loss_exp

        
        # loss_val = loss_PDE + loss_B_top + loss_B + loss_NB_T
        # print("Loss_uvw",loss_uvw.cpu().detach().numpy())
        # print("Loss_T",loss_T.cpu().detach().numpy())
