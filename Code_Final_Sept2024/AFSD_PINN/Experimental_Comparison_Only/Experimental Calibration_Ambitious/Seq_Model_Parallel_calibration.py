...
class coupled_PINN(nn.Module):
    ...
    def __init__(self,layers1,layers2,device1,device2,lb_xyz,ub_xyz,sheet_name):
        super().__init__() #call __init__ from parent class 
        ...
        # self.T_exp = torch.from_numpy(data_exp).float().to(device2)
        # self.xyz_exp = torch.from_numpy(xyz_exp).float().to(device2)
        
        #LATE ADDITION
        mid_idx = range(17)
        self.T_exp = torch.from_numpy(data_exp[mid_idx,:]).float().to(device2)
        self.xyz_exp = torch.from_numpy(xyz_exp[mid_idx,:]).float().to(device2)

        # self.PINN_T.C_bot = Parameter(torch.tensor(0.15,device = device2))
        # self.PINN_T.C_bot.requiresGrad = True

        # self.PINN_T.k_c = Parameter(torch.tensor(0.15,device = device2))
        # self.PINN_T.k_c.requiresGrad = True

        self.PINN_T.C_bot = torch.tensor(0.15,device = device2)
        self.PINN_T.k_c = torch.tensor(0.0,device = device2)
        # self.h_sides = Parameter(torch.tensor(5.0))
        # self.h_sides.requiresGrad = True
    ...
    def helper_gradients(self,g,ind):
        ...
        if(ind==0):    
            ...
            if(ind==1):
                ...
                k = k0 - self.PINN_T.k_c*(T.detach()-T_a).reshape(-1,)/1000
                kT_xx = k*T_x_xyz[:,0]
                kT_yy = k*T_y_xyz[:,1]
                kT_zz = k*T_z_xyz[:,2]
    ...
    def loss_NB_T_5sides(self,xyz_1, xyz_2, xyz_3, xyz_4, xyz_bot):
        ...
        k = k0 - self.PINN_T.k_c*(T.detach()-T_a).reshape(-1,1)/1000
        ...
        k = k0 - self.PINN_T.k_c*(T.detach()-T_a).reshape(-1,1)/1000
        f = k*T_z + self.PINN_T.C_bot*1e-6*torch.pow(T_a - T,3)
        # f = k*T_z + *torch.pow(T_a - T,3)
        f_true = 0.0*f.detach()
    ...
    def loss_NB_T_5sides_v2(self,xyz_1, xyz_2, xyz_3, xyz_4, xyz_bot):
        ...
        T_z = T_xyz[:,2:3]
        k = k0 - self.PINN_T.k_c*(T.detach()-T_a).reshape(-1,1)/1000
        f5 = k*T_z + self.PINN_T.C_bot*(1e-6*torch.pow(T_a - T,3))
    ...
    def loss_exp_data(self):
        T_pred = self.PINN_T.forward(self.xyz_exp)
    ...
    def loss(self,xyz_coll,xyz_1, xyz_2, xyz_3, xyz_4,xyz_top,xyz_bot,f_hat,N_hat,uvw_true_top,r_fr_ph_out):
        ...
        loss_exp = self.loss_exp_data()
#         print(loss_PDE.cpu().detach().numpy())
#         print(loss_B_top.cpu().detach().numpy())sads
#         print(loss_B.cpu().detach().numpy())
        
        # loss_T_np = loss_exp.cpu().detach().numpy()
        loss_exp_np = loss_exp.cpu().detach().numpy()
        # loss_val = loss_PDE + loss_B_top + loss_B + loss_NB_T
        # print((loss_uvw.cpu().detach().numpy()/loss_T.cpu().detach().numpy()))
        print("Loss ",loss_exp_np,"C_bot ",self.PINN_T.C_bot.cpu().detach().numpy(),"k_c",self.PINN_T.k_c.cpu().detach().numpy())


        # if (loss_exp_np <10):
            # loss_T_full = loss_T+ 0.001*loss_exp
        # else:
            # loss_T_full = loss_T + 0.001*loss_exp

        return loss_uvw, loss_T + loss_exp 
